...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $targetFolder = "../../uploads/associate_resort/";
    $image_url = $targetFolder . basename($_FILES["image_url"]["name"]);
    $image_path = "uploads/associate_resort/" . basename($_FILES["image_url"]["name"]);

    if (move_uploaded_file($_FILES["image_url"]["tmp_name"], $image_url)) {
        $stmt = $conn->prepare("INSERT INTO associate_resort (location_name, type, resort_name, image_url, free_wifi, reservation, credit_card, air_conditioner, car_parking, swimming_pool, overview)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>window.location.href = '../all_associated_resort.php';</script>";
        } else {
            echo "<script>alert('Not Saved!');</script>";
            echo "<script>window.location.href = '../add_resort.php';</script>";
        }
